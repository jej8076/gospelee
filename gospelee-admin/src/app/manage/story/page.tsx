...
export default function ManageStory(): JSX.Element {
  ...
  const handleEdit = (announcement: Announcement) => {
    if (announcement.id) {
      router.push(`/manage/story/edit/${announcement.id}`);
    }
  };
  ...
  const loadAnnouncements = async () => {
    try {
      setIsLoading(true);
      await callApi(fetchAnnouncements, setAnnouncements, 'BRAND_STORY');
    } catch (error) {
      console.error('브랜드 스토리 로드 실패:', error);
    } finally {
      setIsLoading(false);
    }
  };
