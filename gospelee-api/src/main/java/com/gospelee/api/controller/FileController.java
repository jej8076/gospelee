...
public class FileController {
   ...
   * @return 파일 리소스
   */
  @GetMapping("/{accessToken}/{fileDetailId}")
  ...
  /**
   * 보안 파일 다운로드 API (accessToken 기반) accessToken과 file_details ID를 조합하여 안전하게 파일을 다운로드합니다.
   *
   * @param accessToken  파일 접근 토큰 (UUID)
   * @param fileDetailId file_details 테이블의 ID
   * @return 파일 리소스 (다운로드)
   */
  @GetMapping("/download/{accessToken}/{fileDetailId}")
